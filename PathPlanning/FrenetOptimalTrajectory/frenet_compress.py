...
MAX_ROAD_WIDTH = 25  # maximum road width [m]
...
def main():
    ...
    global MAX_SPEED, TARGET_SPEED, D_T_S
    MAX_SPEED = args.v_max
    # MAX_ACCEL = 2.0  # maximum acceleration [m/ss]
    # MAX_CURVATURE = 1.0  # maximum curvature [1/m]
    ...
    TARGET_SPEED = args.v_init
    # D_T_S = 5.0 / 3.6  # target speed sampling length [m/s]
    # N_S_SAMPLE = 1  # sampling number of target speed
    # D_T_S = MAX_SPEED / N_S_SAMPLE
    ...
    N = 20
    R = 0
    wx = np.linspace(0, L, N)
    wy = [R * math.sin(math.pi / (2.0 * L / N) * x) for x in wx]

    obs = []
    i = 1
    d_T = 10
    while (args.v_init * d_T) * i < L:
        print(i)
        offset = 1
        if i % 2 == 0:
            offset = -1
        ...
        for d_y in [5.0, 10, 15.0, 20.0]:
            obs.append([(args.v_init * d_T) * i, offset * d_y])

        obs.append([(args.v_init * d_T) * i, 0])
        # obs.append([(args.v_init * 10) * i, -offset * 2.5])
        i = i + 1
    ...
    # initial state
    c_speed = args.v_init  # current speed [m/s]
    ...
    for i in range(SIM_LOOP):
        ...
        if np.hypot(path.x[1] - tx[-1], path.y[1] - ty[-1]) <= 5.0:
            print("Goal")
            break
